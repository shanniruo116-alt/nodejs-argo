...
const PROJECT_URL = process.env.PROJECT_URL || 'https://google.com';    // 需要上传订阅或保活时需填写项目分配的url,例如：https://google.com
...
const UUID = process.env.UUID || 'eb7db1ee-3ef8-4545-94db-346146706ce9'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
...
async function generateLinks(argoDomain) {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      // 注意：VMESS 链接参数需根据你使用的客户端插件进行调整，目前标准做法是将 net 改为 xhttp
      const VMESS = { v: '2', ps: `${nodeName}`, add: CFIP, port: CFPORT, id: UUID, aid: '0', scy: 'none', net: 'xhttp', type: 'packet', host: argoDomain, path: '/vmess-argo?ed=2560', tls: 'tls', sni: argoDomain, alpn: 'h2,h3', fp: 'firefox'};
      
      const subTxt = `
vless://${UUID}@${CFIP}:${CFPORT}?encryption=none&security=tls&sni=${argoDomain}&fp=firefox&type=xhttp&mode=packet&host=${argoDomain}&path=%2Fvless-argo%3Fed%3D2560#${nodeName}

vmess://${Buffer.from(JSON.stringify(VMESS)).toString('base64')}

trojan://${UUID}@${CFIP}:${CFPORT}?security=tls&sni=${argoDomain}&fp=firefox&type=xhttp&mode=packet&host=${argoDomain}&path=%2Ftrojan-argo%3Fed%3D2560#${nodeName}
    `;
      // 打印 sub.txt 内容到控制台
